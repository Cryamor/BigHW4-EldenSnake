...
int Snake::autoMove(Game& game, Food& food)
{
	POINT target = food.deq.front();
	int x1 = this->getHeadX(), y1 = this->getHeadY();
	...
	for (int i = 0; i < map.size(); i++) {
		map[i].resize(MAP_A);//每一行都是N列
	}
	for (int y = 0; y < MAP_A; y++) {
		for (int x = 0; x < MAP_A; x++) {
			if (game.map[y * MAP_A + x] == MAP_FOOD || game.map[y * MAP_A + x] == MAP_VOID)
				map[y][x] = 0;
			else
				map[y][x] = 1;
		}
	...
	path.pop_front();
	int x2 = path.front()->y, y2 = path.front()->x;
